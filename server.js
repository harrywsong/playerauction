...
io.on('connection', (socket) => {
  ...
  function tryAutoAssignEliminationIfOnlyOneOption(room) {
    if (!room.settings.limitByTier) return false;
    if (room.elimination.length === 0) return false;
  
    // Get the next player to be auctioned in elimination
    const next = room.elimination[0];
    const tier = next.tier;
  
    // 1. Is this the ONLY remaining player of this tier in elimination?
    const unassignedOfThisTier = room.elimination.filter(e => e.tier === tier);
    if (unassignedOfThisTier.length !== 1) return false;
    ...
    if (eligibleTeams.length === 1) {
      ...
      // Remove from elimination
      room.elimination = room.elimination.filter(e => e.username !== next.username);
  ...
  socket.on('start_elim_auction', ({ roomCode }) => {
    ...
    // ====== AUTO-ASSIGN CHECK FOR ELIMINATION ======
    // Attach roomCode property so helper can access it
    room.roomCode = roomCode;
    if (tryAutoAssignEliminationIfOnlyOneOption(room)) {
      // If auto-assigned, skip the auction start entirely
      return;
    }
    // ====== END AUTO-ASSIGN CHECK ======
...
>>>>>>> 26316e305236d651758e107d090d070b544eae56
